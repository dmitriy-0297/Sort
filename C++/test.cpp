...
int main(){
  ...
  for(int k = 0; k < div_vec; k++){ // методом тестирования выявил, что для полной сортировки достаточно количества делений,
    //если брать больше - не оптимально, меньше - неполность сортируется массив
  for(int i = 0; i < div_vec-1; ++i){
      if ((v.at(i).at((n/div_vec - 1)) >= v.at(i+1).at(0)) || (v.at(i).at(0) > v.at(i+1).at(0))
      || (v.at(i).at(n/div_vec - 1) < v.at(i+1).at(n/div_vec - 1)) || (v.at(i).at(0) <= v.at(i+1).at(n/div_vec - 1))){
